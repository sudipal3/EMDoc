...
function addText(text, button) {
    ...
    if (sectionName === 'ultrasound' && button.innerText === 'US used') {
...
function generateAdditionalProcedureNote() {
    ...
    additionalNote.innerHTML = `
        <h3>Procedure: Bedside ultrasound guidance for regional nerve block</h3>
        <p><strong>Indication:</strong> ultrasound guidance for regional nerve block placement for pain control and/or procedural analgesia.</p>
        <p><strong>Procedure:</strong> limited ultrasound of the target nerve and surrounding anatomy to guide needle placement.</p>
        <p><strong>Interpretation:</strong> target nerve and surrounding structures were visualized dynamically during the procedure and the needle tip was identified adjacent to the nerve in real time. Images were saved in the medical record and/or PACS as available.</p>
    `;
