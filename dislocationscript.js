// Define the order of sections for consistent output ordering
const sectionOrder = [
    'consent',
    'locationType',
    'preReductionExam',
...
const dislocationNoteHeader = `Procedure Note: Dislocation Reduction`;
...
function addText(text, button) {
    ...
    // Add the main header if not already present
    addMainHeader();
}
...
// Function to add the main Procedure Note header
function addMainHeader() {
    const outputArea = document.getElementById('outputArea');
    if (!outputArea.querySelector('h2')) {
        outputArea.insertAdjacentHTML('afterbegin', `<h2>${dislocationNoteHeader}</h2><br>`);
    }
}
